...
/* 설정 파일의 userMethodNameResolver 프로퍼티를 사용하려면 반드시 MultiActionController를 
   상속받아야 함 */
public class UserController extends MultiActionController {
	...
	*/
	...
	throws Exception {
		...
		// getViewName() 메서드를 호출해서 요청명에서 확장명 .do를 제외한 뷰이름을 가져옴
	...
	throws Exception {
	    ...
	    /* 회원 가입창에서 전송된 회원 정보를 addObject() 메서드를 이용해 ModelAndView 
	       객체에 바인딩 */
	    mav.addObject("id",id);
	...
	private String getViewName(HttpServletRequest request) throws Exception {
	     ...
	     * 다시 http://localhost:8090/member/listMember에서 역순으로 
	     * 첫번째 '/' 위치를 구한 후, 그 뒤의 listMember를 구한다. */
