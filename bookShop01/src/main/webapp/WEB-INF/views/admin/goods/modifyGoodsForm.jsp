...
function fn_modify_goods(goods_id, attribute) {
  ...
  // 기존 이미지를 다른 이미지로 변경한 후 FormData를 이용해 Ajax로 수정
  function modifyImageFile(fileId,goods_id, image_id,fileType) {
    // alert(fileId);
	  var form = $('#FILE_FORM')[0];
      var formData = new FormData(form);
      
      // formData에 수정할 이미지와 이미지 정보를 name/value로 저장
      ...
      $.ajax({
        ...
        processData: false,
        contentType: false,
        data: formData,		// formData를 Ajax로 전송
        type: 'POST',
  ...
  // 새 이미지 추가 후 FormData를 이용해 Ajax로 수정
